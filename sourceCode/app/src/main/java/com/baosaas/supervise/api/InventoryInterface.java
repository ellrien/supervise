...
public class InventoryInterface extends BaseWebService {
     ...
     * @return
     */
    public InventoryBillOut getInventoryTask(UserBillIn user) {
        //test
//        user.setToken("49218165271a4be0b14667f51d8cba9c");
//        user.setUserId("10086");
        //end of test
        String json = JSON.toJSONString(user);
        String result = getDataBySoapObjectJson(json, "getInventoryTask");
...
//        Log.e("getBillList", result);
     ...
     */
    //上传已盘数据
    public UploadOut updateInventoryTask(UploadBills bills){
        String json= JSON.toJSONString(bills);
        String result=getDataBySoapObjectJson(json, "updateInventoryTask");
        UploadOut out=null;
        if (StringUtils.isNotEmpty(result)){
            out=JSON.parseObject(result,UploadOut.class);
        }
        return out;
    }
}
